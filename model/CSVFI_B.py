...
class SynBlock(nn.Module):
    ...
    def forward(self, fea, frames, edges, output_size):
        ...
        bia = torch.cat(torch.unbind(fea, 1), 1)  # 对特征按T进行切片，按通道C拼接
        bia = self.lrelu(self.feature_fuse(bia))  # 特征融合 将T信息融合起来 B C H W
        bias = self.ModuleB(bia, (H, W))  # 单独生成残差
        ...
        # M_s = self.ModuleM(fea, (H, W)).view(B, T, -1, H, W)
