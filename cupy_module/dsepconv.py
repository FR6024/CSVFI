...
class _FunctionDSepconv(torch.autograd.Function):
    ...
    """
    torch.Size([1, 3, 68, 116]) input
    torch.Size([1, 5, 64, 112]) vertical
    torch.Size([1, 5, 64, 112]) horizontal
    torch.Size([1, 25, 64, 112]) offset_x
    torch.Size([1, 25, 64, 112]) offset_y
    torch.Size([1, 25, 64, 112]) mask
    """

    @staticmethod
    def forward(self, input, vertical, horizontal, offset_x, offset_y, mask):
        self.save_for_backward(input, vertical, horizontal, offset_x, offset_y, mask)  # 存储 用来反向传播
